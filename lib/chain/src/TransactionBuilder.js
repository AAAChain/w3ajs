...
class TransactionBuilder {
    ...
    */
    ...
    // add signature generated by user account
    addSignature(signature) {
        ...
        this.signatures.push(signature.toBuffer());
        this.signer_private_keys = [];
        this.signed = true;
    }
    serialize() {
...
function _broadcast(was_broadcast_callback) {
    return new Promise((resolve, reject) => {
        ...
        let thxId = this.id(); // get transation id
        var tr_object = ops.signed_transaction.toObject(this);
        Apis.instance()
            .network_api()
            ...
            .then(function() {
                if (was_broadcast_callback) was_broadcast_callback(thxId);
            })
            .catch(error => {
